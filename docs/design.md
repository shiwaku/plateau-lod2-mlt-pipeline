...
### 8.5 入力データ側の欠落（霞が関の官公庁）

霞が関 2 丁目付近（外務省 139.7484/35.6740 の周辺、東西 300 m × 南北 350 m）で建物が表示されないという指摘を受けて調べた結果、**元の CityGML にその範囲の大きな建物が含まれていない**ことが分かった。

- 範囲内の `bldg:Building` は 11 棟のみで、いずれも面積 数〜数百 m² の小さな建物（用途 454「その他」、461「不明」）。外務省・財務省・国土交通省・総務省などの庁舎は建物として存在しない。
- 霞が関・永田町一帯（139.744〜139.756、35.669〜35.680）で用途 421「官公庁施設」は 35 棟あるが、大きなものは 法務省周辺の 9,292 m²（93 m）、中央合同庁舎の 6,767 m²（120 m）、霞が関 1 丁目の 5,199 m²（75.5 m）など一部に限られる。
- NDJSON とタイルは元データどおりで、変換・タイル生成での欠落ではない（同範囲の 23 面はすべて収録、高さ 0 の面も無い）。

対応は本パイプラインの範囲外。必要なら他ソース（最適化ベクトルタイルの `BldA` 建物ポリゴンや OpenStreetMap）で補完する案があるが、高さ情報が無いか精度が異なるため別途検討とする。
