...
6. ビューアでの表示確認時、GSI 最適化ベクトルタイル（背景）の読み込みに 30 秒以上かかることがあった。建物タイルの問題ではない（最小スタイルなら数秒で描画される）。
   ビューアには `?base-map=none`（パネルの「背景地図」チェックボックス）で背景なしに切り替える手段と、1.5 秒待っても描画が終わらないときに出る「背景地図を読み込み中…／背景なしで表示」の表示を入れた

## 4.1 ビューアの操作

| 操作 | 内容 |
| --- | --- |
| データセット / MLT・MVT の切り替え | 建物ソースだけを差し替える。背景地図は読み込んだまま維持される |
| 背景地図 | チェックを外すか `?base-map=none` で GSI 最適化ベクトルタイルを読まない。読み込みが長引くと画面下に切り替えボタンが出る |
| 凡例のチェックボックス | `kind` ごとの表示切り替え。「外殻を隠す」で屋根・設備・壁・開口部・地面を落とし、床・天井・Room だけにする（竹芝 LOD4 の階ごとの床板はこれで見える） |

なお、GSI スタイルは sprite / glyphs の読み込みが終わるまで `load` / `styledata` が発火しないため、建物レイヤーは
スタイルに焼き込んで初期化し、切り替え時だけ `addSource` / `addLayer` で差し替えている。
